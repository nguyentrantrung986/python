...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    for cuisine in cuisines_list:
        for name in cuisine_to_names[cuisine]:
            if name in names_matching_price and name not in rest:
                rest.append(name)
    return rest
...
def read_restaurants(file):
    ...
    while True:
        rest_name = file.readline().strip()
        rate = int(file.readline().strip()[:-2])
        price = file.readline().strip()
        cuisines = file.readline().strip().split(sep=',')

        name_to_rating[rest_name] = rate
        price_to_names[price].append(rest_name)
